...
import { dataStorageContractABI } from "../../../contracts/dataStorageContractABI";
...
function ViewAssetsForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (willContract === "0x0000000000000000000000000000000000000000") {
        ...
        await fetchAssets(willContract);
        setLoading(LoadingState.FETCH_COMPLETE);
      }
    };
  ...
  const fetchAssets = async (willContract) => {
    console.log("fetching assets...");
    try {
      // fetch data storage
      const dataStorageContract = await Moralis.executeFunction({
        contractAddress: willContractAddress || willContract,
        functionName: "getDataStorageContract",
        abi: willContractABI,
      });

      const tokenIdList = await Moralis.executeFunction({
        contractAddress: willContractAddress || willContract,
      ...
      for (let i = 0; i < tokenIdList.length; i++) {
        const assetData = await Moralis.executeFunction({
          contractAddress: dataStorageContract,
          functionName: "getTokenIdData",
          abi: dataStorageContractABI,
          params: { tokenId: tokenIdList[i] },
        });

        console.log("fetched Asset: ", assetData);
        fetchedAssets.push({
          tokenId: tokenIdList[i].toNumber(),
          assetNftContractAddress: assetData[0],
          beneficiary: assetData[2],
        });
      }
  ...
  const handleDestroy = async (willContract) => {
    try {
      ...
      const txReceipt = await destroyWillTx.wait();
      ...
      if (txReceipt.events[0]) {
        ...
        setLoading(LoadingState.SUBMIT_FAILURE);
      }
    } catch (error) {
  ...
  return (
    <div style={styles.card}>
      {isExecuted ? (
        ...
        <div>
          <Alert
            message="Error 404: Will not found"
            description="Will has not been created or is destroyed..."
            type="error"
            closable
          />
        </div>
        ...
        <div>
          {!isZeroAddress && (
            <div>
              ...
              loading === LoadingState.SUBMIT_FAILURE ? (
                ...
                >
                  {loading === LoadingState.SUBMIT_LOADING ? (
                    ...
                    // success
                    <div>
                      <div style={{ textAlign: "center" }}>Will DESTROYED</div>
                      <div
                        style={{ textAlign: "center", fontWeight: "normal" }}
                      >
                        View will on{" "}
                              ...
                              >
                                <Space direction="vertical">
                                  ...
                                  <Address
                                    size={4}
                                    copyable
                                    address={asset.assetNftContractAddress}
                                  />
                                </Space>
                                {/*<DeleteTwoTone*/}
                                {/*  twoToneColor={red[3]}*/}
